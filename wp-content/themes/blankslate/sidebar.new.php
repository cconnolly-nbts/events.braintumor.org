...
<?php } ?>
<?php if (($donations && $donations == 'false') || ('events' == get_post_type() && $donations == '')){} else { ?><a class="<?php if ('events' == get_post_type()) { ?>donateButton<?php } else { ?>registerButton<?php } ?> pure-button" href="<?php if ('events' == get_post_type()) { ?>http://www.braintumorcommunity.org/site/TR/Events/?fr_id=<?php echo $fr_id; ?>&pg=pfind<?php } else { ?>/donate/<?php } ?>"><strong>DONATE</strong><?php if ('events' == get_post_type()) { echo ' TO A PARTICIPANT';} ?></a><?php } ?>
